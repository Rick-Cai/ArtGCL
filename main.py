...
print('artist_num:',train.shape[0],'lambda_1:',lambda_1,'lambda_2:',lambda_2,'temp:',temp,'q:',svd_q)
...
for i in tqdm(range(len(train.data))):
    train.data[i] = math.log(train.data[i]+1) / pow(rowD[train.row[i]]*colD[train.col[i]], 0.5)
...
model = SimGCL(adj_norm.shape[0], adj_norm.shape[1], d, train_csr, adj_norm, l, temp, lambda_1, dropout, batch_user, device)
