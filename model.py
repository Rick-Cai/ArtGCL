...
class SimGCL(nn.Module):
    def __init__(self, n_u, n_i, d, train_csr, adj_norm, l, temp, lambda_1, dropout, batch_user, device):
        ...
        # self.G_u_1_list = [None] * (l+1)
        # self.G_u_2_list = [None] * (l+1)
        # self.G_u_1_list[0] = self.E_u_0
        # self.G_u_2_list[0] = self.E_u_0
    ...
    def forward(self, uids, iids, pos, neg, test=False):
        if test==True:  # testing phase
            preds = self.E_u[uids] @ self.E_u.T
            ...
            for layer in range(1,self.l+1):
                # GNN propagation
                self.Z_u_list[layer] = self.act(spmm(self.adj_norm, self.E_u_list[layer-1], self.device))
                ...
                random_noise = torch.randn_like(self.Z_u_list[layer]).cuda(torch.device(self.device))
                self.Z_u_list[layer] += torch.sign(self.Z_u_list[layer]) * nn.functional.normalize(random_noise, dim=-1) * self.eps
                
                # svd_adj propagation
                # vt_ei = self.vt @ self.E_u_list[layer-1]
                # self.G_u_list[layer] = self.act(self.u_mul_s @ vt_ei)
                #ut_eu = self.ut @ self.E_u_list[layer-1]
                #self.G_i_list[layer] = self.act(self.v_mul_s @ ut_eu)
                # self.G_u_1_list[layer] = self.act(spmm(self.adj_norm, self.G_u_1_list[layer-1], self.device))
                # random_noise = torch.randn_like(self.G_u_1_list[layer]).cuda(torch.device(self.device))
                # self.G_u_1_list[layer] += torch.sign(self.G_u_1_list[layer]) * nn.functional.normalize(random_noise, dim=-1) * self.eps

                # self.G_u_2_list[layer] = self.act(spmm(self.adj_norm, self.G_u_2_list[layer-1], self.device))
                # random_noise = torch.randn_like(self.G_u_2_list[layer]).cuda(torch.device(self.device))
                # self.G_u_2_list[layer] += torch.sign(self.G_u_2_list[layer]) * nn.functional.normalize(random_noise, dim=-1) * self.eps
            ...
            gnn_u = nn.functional.normalize(self.E_u[uids],p=2,dim=1)
            hyper_u = nn.functional.normalize(self.Z_u_list[1][uids],p=2,dim=1)
            hyper_u = self.Ws[0](hyper_u)
            ...
            for i in range(len(uids)):
                ...
                u_pos = pos[i]
                u_neg = neg[i]
                pos_emb = self.E_u[u_pos]
                neg_emb = self.E_u[u_neg]
